  ...
  $.fn.simpleLens = function (options) {
    ...
    var lens = {
      ...
      calc_lens_position: function (cursor_position) {
        return {
          left:
            (cursor_position.center_left * this.ratio.x -
              this.container.width) *
            -1,
          top:
            (cursor_position.center_top * this.ratio.y -
      ...
      lens_event_bind: function () {
        ...
        var mouse_callback = function (mouse) {
          ...
          that.update_lens_position(
            that.calc_lens_position(
      ...
      destroy: function () {
        ...
        if (
          this.lens_container !== undefined &&
          this.lens_container.length > 0
        ) {
      ...
      load: function (e) {
        ...
        var margin_top = this.lens_container.height() / 2 - 25,
          loading_image = $("<img>", { src: opts.loading_image }).css(
            "margin-top",
            margin_top
          );
        ...
        this.lens_image
          .load(function () {
            ...
            that.update_lens_position(
              that.calc_lens_position(cursor.cursor_position)
            );
    ...
    $(this)
      .parents(opts.parent_class)
      .on(opts.open_lens_event, opts.anchor_parent_class, init);
    $(this)
      .parents(opts.parent_class)
      .on("mouseleave", opts.anchor_parent_class, destroy);
  };
